# browser-provider-implementer

> 网页端大模型对接路由开发全流程。触发条件：浏览器网页模型对接、Playwright转发、内网穿透端口配置、IDE代理调试、接口通信异常修复。兼容豆包/千问/Kimi/DeepSeek等所有网页端模型。
...
await pool.register(
    ...
    model="xxx-web",
    auth_state_file="xxx_auth_state.json",  # 如果需要登录态
)
```
...
| 登录态文件 | `lama-puppeteer/xxx_auth_state.json` | 模型名 + `_auth_state.json` |
...
cd santian/ai-skills/.lingma/skills/browser-provider-implementer
`
